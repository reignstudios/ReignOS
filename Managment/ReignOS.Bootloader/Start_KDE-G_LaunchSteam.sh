...
if [ "$REIGN_MONITOR" = "true" ]; then
  /home/gamer/ReignOS/Managment/ReignOS.Monitor/bin/Release/net10.0/linux-x64/publish/ReignOS.Monitor &
fi
...
if [ "$DISABLE_STEAM_DECK" = "true" ]; then
    if [ "$USE_MANGOHUB" = "true" ]; then
        MANGOHUD_CONFIG="horizontal,position=top-center,hud_no_margin,horizontal_stretch,table_columns=100,cpu_power,gpu_power,cpu_temp,gpu_temp,battery,battery_watt" \
