...
namespace ReignOS.Service.Hardware
{
    public static class Asus
    {
        ...
        {
            isEnabled = false;
            bool initGamepad = false, preferInputDevice = false;
            ushort vid = 0, pid = 0;
            ...
            else if (Program.hardwareType == HardwareType.RogAllyX || Program.hardwareType == HardwareType.RogXboxAllyX || Program.hardwareType == HardwareType.RogXboxAlly)
            {
                isEnabled = true;
                initGamepad = true;
                preferInputDevice = Program.hardwareType == HardwareType.RogXboxAllyX || Program.hardwareType == HardwareType.RogXboxAlly;
                vid = 0x0b05;
                pid = 0x1b4c;
            }
            ...
            if (initGamepad)
            {
                ...
                if (preferInputDevice)
                {
                    inputDevice = new KeyboardDevice();
                    inputDevice.Init(null, vid, pid, exclusiveLock:true, initAsGamepad:true);
        ...
        private static void ProcessGamepadInput(Gamepad gamepad)
        {
